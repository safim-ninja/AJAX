...
<body onload="index()">
    ...
    <script>
        function index() {
            ...
            zhttp.open("GET", "http://127.0.0.1:8000/api/fruits", true);
            zhttp.setRequestHeader('Content-type', 'application/json');
            zhttp.send();
            var publicPath = "{{ asset('images/') }}";
            ...
            zhttp.onreadystatechange = function() {
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        table += "<tr style=\"height: 100px\"><th scope=\"row\">" + (i + 1) + "</th><td>" + data[i]
                            .name + "</td><td>" + data[i]
                            .price + "</td><td><img style=\"width: 100px\" src=\"" + publicPath + "/" + data[i].image +
                            "\"></img></td><td><button type=\"button\" class=\"btn py-0 px-2 mb-2\" onclick=\"edit(" +
                            data[i]
                            .id +
        ...
        function edit(id) {
            ...
            zhttp.open("GET", "http://127.0.0.1:8000/api/fruits/" + id + "/edit", true);
            zhttp.setRequestHeader('Content-type', 'application/json');
            zhttp.send();
            zhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
        ...
        function update(id) {
            ...
            zhttp.open("PATCH", "http://127.0.0.1:8000/api/fruits/" + id, true);
            zhttp.setRequestHeader('Content-type', 'application/json');
        ...
        function destroy(id) {
            var zhttp = new XMLHttpRequest();
            zhttp.open("DELETE", "http://127.0.0.1:8000/api/fruits/" + id, true);
            zhttp.setRequestHeader('Content-type', 'application/json');
            zhttp.send();
            zhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
